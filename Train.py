...
class Train_Model():
	...
	def create_dataset(self,samples,cam,dataset_name):
		...
		while (True):
			...
			faces = self._Face_Cascade.detectMultiScale(gray,scaleFactor = 1.098, minNeighbors = 8, minSize = (50, 50))
			...
			try :
				for _,face in enumerate(faces):
					...
					Right_Eye = self._Right_Eye_Cascade.detectMultiScale (gray[y: y + int (h / 2), x: x + int (w / 2)],
						scaleFactor = 1.01, minNeighbors = 8)
					...
					for _,eye1 in enumerate(Right_Eye):
						...
						for _,eye2 in enumerate(Left_Eye):
							...
							eyeYdis = (ly + lh * 0.666) - (ry + rh * 0.666)
							...
							# Image rotation 
							rotated_image = imutils.rotate_bound (gray_chunk, (-1) * angle_degree)
